...
class WorldMapPortfolio {
    ...
    setupGeolocation() {
        ...
        // Géolocalisation manuelle au clic
    ...
    geolocateUser(isAuto = false) {
        const geolocateBtn = document.getElementById('geolocateBtn');
        if (!geolocateBtn) return;

        // Afficher l'état de chargement
        geolocateBtn.classList.add('loading');
        geolocateBtn.querySelector('.geolocate-text').textContent = isAuto ? 'Localisation...' : 'Localisation...';
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                // Succès
                this.onGeolocationSuccess(position, geolocateBtn);
            },
            (error) => {
                // Erreur
                this.onGeolocationError(error, geolocateBtn, isAuto);
            },
            options
        );
    }

    onGeolocationSuccess(position, button) {
        ...
        button.querySelector('.geolocate-text').textContent = 'Localisé !';
        
        // Afficher un message de succès temporaire
        setTimeout(() => {
            button.querySelector('.geolocate-text').textContent = 'Me localiser';
        }, 2000);
    }

    onGeolocationError(error, button, isAuto) {
        // Restaurer le bouton
        button.classList.remove('loading');
        button.querySelector('.geolocate-text').textContent = 'Me localiser';
        ...
        // Afficher l'erreur seulement si ce n'est pas une géolocalisation automatique
        if (!isAuto) {
            this.showGeolocationError(errorMessage);
        }
        
        console.log('Erreur de géolocalisation:', errorMessage);
    }

    centerMapOnUser(lat, lng) {
        if (!this.map) return;
        
        // Centrer la carte sur l'utilisateur avec une animation fluide
        this.map.setView([lat, lng], 8, {
            animate: true,
            duration: 1.5
        });
        
        // Ajouter un marqueur temporaire pour l'utilisateur
    ...
    addUserMarker(lat, lng) {
        ...
        this.userMarker.bindPopup('Votre position actuelle', {
            closeButton: false,
    ...
    showGeolocationError(message) {
        ...
        notification.className = 'geolocation-error';
